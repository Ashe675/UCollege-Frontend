...
import { getOBSMessage, getTextOBSColor } from "@/utils/dictionaries";
...
export default function HistoryAcademicStudent({
  data: student,
}: HistoryAcademicStudentProps) {
  return (
    <div className="p-4 bg-gray-100 w-full rounded-sm">
      <div className="flex items-center gap-4 mb-6 bg-white p-4 rounded-lg shadow-md flex-wrap justify-around">
        <Avatar
          img={student.avatar[0]?.url ? student.avatar[0].url : ""}
          rounded
          size="xl"
        />
        <div>
          <h1 className="text-2xl font-semibold text-gray-800">
            {student.nameStudent.replace("null", "")}{" "}
            {student.lastnameStudent.replace("null", "")}
          </h1>
          <p className="text-gray-600">
            Código de Identificación: {student.codeIdentification}
          </p>
          <p className="text-gray-600">
            Centro Regional: {student.regionalCenter}
          </p>
          <p className="text-gray-600">Carrera: {student.career}</p>
          <p className="text-gray-600">
            Promedio Global: {student.globalAverage}
          </p>
          <p className="text-gray-600">
            Índice Académico: {student.academicIndex ?? "N/A"}
          </p>
        </div>
      </div>
              ...
              {data.enrollments.map((enrollment) => (
                ...
                >
                  ...
                  <p className={`text-gray-600 `}>
                    ...
                    <span
                      className={`${getTextOBSColor(
                        enrollment.obs as ObservationEnum
                      )}`}
                    >
                      ({getOBSMessage(enrollment.obs as ObservationEnum)})
                    </span>
                  </p>
                </li>
              ))}
